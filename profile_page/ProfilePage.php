...
<header>
    ...
    <nav>
        <ul class="top_links">
            <li><a href="editpage.php">Account Preferences</a>
...
if (isset($_POST['reg_user'])) {
   ...
   if (count($errors) == 0) {
    ...
    $query = "SELECT * FROM admin WHERE username='$user' AND passcode='$cp'";
    $results = mysqli_query($db, $query);
    if (mysqli_num_rows($results) == 1) {
    $password = md5($password_1);
    $query = "UPDATE admin SET passcode='$password' WHERE username = '$user'";
